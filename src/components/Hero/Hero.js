import React from 'react';
import './Hero.css'


const Hero = () => {
    return (
        <div>
            <div className='flex justify-evenly items-center'>
           <div className='pl-8 text-start'> 
            <h1 className='text-5xl font-bold pb-10'>We care about the <br /> IT Solution you need</h1>
            <p className='text-2xl opacity-50 pb-10'>Suffix IT provides best and prominent Software & <br /> Hardware solutions to ensure your business</p>
           <div className='flex'>
            <button className='btn'>LET'S EXPLORE</button>
            
            <img className='ml-20 h-10' src="data:image/png;base64," alt="" />
           
           </div>
           </div>
           <div>
            <img className='banner-img' src="https://suffixit.com/static/media/banner_image.a079af09.png" alt="" />
            <img className='dot-down' src="data:image/png;base64," alt="" />
            <img className='dot-up' src="data:image/png;base64," alt="" />
            <div>
                <span className='bubble-1'></span>
                <span className='bubble-2'></span>
            </div>
           </div>
           </div>
        </div>
    );
};

export default Hero;